...
namespace 自記温度計Tester
{
    ...
    public static class Test7Seg
    {
        ...
        public static List<SegmetSpec> SegmentSpecs;


        public class SegmetSpec
        {
            public NAME name;
            public bool result;
        }
        ...
        {
            SegmentSpecs = new List<SegmetSpec>();
            foreach (var n in Enum.GetValues(typeof(NAME)))
            {
                SegmentSpecs.Add(new SegmetSpec { name = (NAME)n });
            }
        }
        ...
        public static async Task<bool> CheckLum()
        {
            ...
            try
            {
                return await Task<bool>.Run(() =>
                {
                    ...
                    Target232_BT.SendData(Data: "3700ODB,8onOOO", DoAnalysis: false);//7セグ全点灯コマンド送信
                    Thread.Sleep(2500);

                    var blobInfo = General.cam1.blobs.Clone();

                    if (blobInfo == null)//7セグ+LED1～3 合計27ケ
                    {
                        State.VmTestStatus.Spec = "規格値 : 7セグ全点灯";
                        State.VmTestStatus.MeasValue = "計測値 : 全点灯してないDEATH!!!";
                        return false;
                    }
                    ...
                    SegmentSpecs.ForEach(l =>
                    {
                        //テストログの更新
                        State.VmTestStatus.TestLog += l.name.ToString() + "輝度チェック";
                        ...
                        {
                            var pointX = (b.Value.Centroid.X > X - 10) && (b.Value.Centroid.X < X + 10);//X座標の確認
                            var pointY = (b.Value.Centroid.Y > Y - 10) && (b.Value.Centroid.Y < Y + 10);//Y座標の確認
                        ...
                        else
                        {
                            ...
                            //面積の計算
                            re = area >= refLum * (1 - (errLumL / 100.0)) && area <= refLum * (1 + (errLumH / 100.0));
                        }
                    ...
                    return allResult = SegmentSpecs.All(s => s.result);
        ...
        public static async Task<bool> CheckOnOff()
        {
            //電源投入して、RS232C通信をPCモードにしてからメソッドを呼び出すこと
            bool allResult = false;
            int X = 0;
            ...
            try
            {
                return await Task<bool>.Run(() =>
                {
                    ...
                    return allResult = SegmentSpecs.All(l =>
                    {
                        //テストログの更新
                        State.VmTestStatus.TestLog += l.name.ToString() + "点灯チェック";
                        ...
                        //X座標の確認
                        if ((int)pointX < X - 10 || (int)pointX > X + 10) goto FAIL;

                        //Y座標の確認
                        if ((int)pointY < Y - 10 || (int)pointY > Y + 10) goto FAIL;
